...
export const brainSource = pgTable("BrainSource", {
  id: id(),
  ...
  uniqueIndex("BrainSource_provider_key").on(t.provider),
])

export const brainDocument = pgTable("BrainDocument", {
  id: id(),
  sourceId: text("sourceId").notNull().references(() => brainSource.id, { onDelete: "cascade" }),
  ...
  sourceModifiedAt: timestamp("sourceModifiedAt", { withTimezone: true }),
  syncedAt: createdAt("syncedAt"),
  createdAt: createdAt(),
  updatedAt: updatedAt(),
}, (t) => [
  uniqueIndex("BrainDocument_sourceId_externalId_key").on(t.sourceId, t.externalId),
  index("BrainDocument_category_idx").on(t.category),
  index("BrainDocument_kind_idx").on(t.kind),
])

export const brainChunk = pgTable("BrainChunk", {
  id: id(),
  documentId: text("documentId").notNull().references(() => brainDocument.id, { onDelete: "cascade" }),
  index: integer("index").notNull(),
  text: text("text").notNull(),
  tokenCount: integer("tokenCount").notNull().default(0),
  embedding: vector("embedding", { dimensions: 1536 }),
}, (t) => [
  uniqueIndex("BrainChunk_documentId_index_key").on(t.documentId, t.index),
])

export const brainSyncRun = pgTable("BrainSyncRun", {
  id: id(),
  sourceId: text("sourceId").notNull().references(() => brainSource.id, { onDelete: "cascade" }),
  ...
  index("BrainSyncRun_sourceId_startedAt_idx").on(t.sourceId, t.startedAt),
])
...
export type BrainSource = typeof brainSource.$inferSelect
...
export const brainSourceRelations = relations(brainSource, ({ many }) => ({
  documents: many(brainDocument),
  syncRuns: many(brainSyncRun),
}))

export const brainDocumentRelations = relations(brainDocument, ({ one, many }) => ({
  source: one(brainSource, { fields: [brainDocument.sourceId], references: [brainSource.id] }),
  chunks: many(brainChunk),
}))

export const brainChunkRelations = relations(brainChunk, ({ one }) => ({
  document: one(brainDocument, { fields: [brainChunk.documentId], references: [brainDocument.id] }),
}))

export const brainSyncRunRelations = relations(brainSyncRun, ({ one }) => ({
  source: one(brainSource, { fields: [brainSyncRun.sourceId], references: [brainSource.id] }),
}))
